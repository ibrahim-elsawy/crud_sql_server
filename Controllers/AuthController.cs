...
namespace crud_sql_server.Controllers
{
	...
	public class AuthController : ControllerBase
	{
		...
		public async Task<IActionResult> RegisterViewer([FromBody] UserRegistrationDto model)
		{
			...
			await userManager.AddToRoleAsync(user, UserRoles.ViewerUser);

			return Ok(new Response { StatusCode = "Success", Message = "User created successfully!" });
		}
		[HttpPost]
		[Route("register-creator")]
		public async Task<IActionResult> registerCreator([FromBody] UserRegistrationDto model)
		{
			var userExists = await userManager.FindByNameAsync(model.Username);
			if (userExists != null)
				return StatusCode(StatusCodes.Status500InternalServerError, new Response { StatusCode = "Error", Message = "User already exists!" });
			...
			await userManager.AddToRoleAsync(user, UserRoles.CreatorUser);
